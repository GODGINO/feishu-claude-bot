...
export class EmailProcessor {
  ...
  async process(emails: RawEmail[], account: EmailAccount, sessionDir: string): Promise<ProcessedEmail[]> {
    ...
    try {
      return await this.classifyWithClaude(emails, account, userRules);
    } catch (err) {
  ...
  private async classifyWithClaude(emails: RawEmail[], account: EmailAccount, userRules: string): Promise<ProcessedEmail[]> {
...
1. 根据规则判断是否需要推送给用户 (notify: true/false)
2. 用1-2句中文生成摘要 (summary)
